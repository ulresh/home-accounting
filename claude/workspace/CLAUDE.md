...
- **Устройство desktop-стороны** (`src/sync/SyncService.cpp`): одна структура `Session`
  ...
  (`send`/`sendFile`/`flush`, вытесняется в `pumpWrite()` с порогом `kHighWater`, чтобы
  не копить файл в памяти), и шаги протокола, каждый из которых заканчивается вызовом
  продолжения `Cont`. Приём — маленькие автоматы по стадиям (`rwStep`/`riStep`:
  device → people → catalog → event* → end). Отдача целиком синхронная: размеры и
  действующая схема считаются заранее в `scanRange()`, дальше только постановка в очередь.
  Исключения из разбора ловит `Session::call()` и переводит в `res.error`.
