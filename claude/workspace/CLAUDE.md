...
ssh appbuild 'cd .../desktop && ./build/syncv2test'   # 145 unit-тестов модели+синка
...
- `device.jsonl` — `[DN,"<ключ>","<имя устройства>",NN,"disabled"]`. Имя задаёт владелец
  устройства (первый запуск + диалог «Устройства»), NN — счётчик его изменений,
  `"disabled"` = запрет ПРЯМОЙ синхронизации. Слияние: своё имя не меняем никогда,
  имя СОБЕСЕДНИКА принимаем всегда, остальных — только с бо́льшим NN (без NN была бы
  гонка). `"disabled"`, поднятый для НАС в присланном списке, — ошибка протокола:
...
- **Имя своего устройства в hello/`["empty"]`.** Пустой стороне нечего отдавать, а имя
  собеседнику знать надо: клиент шлёт `[код,база,"empty","<имя>"]`, пустой сервер —
  `["empty","<имя>"]`. В остальных случаях имя доезжает в самом `device.jsonl` (смена
  имени меняет sha1 → файл уйдёт при ближайшей синхронизации), поэтому отдельного
  «дослать список» не нужно.
...
- `guitest` — 75/75, `syncv2test` — 145/145 (красных не осталось).
