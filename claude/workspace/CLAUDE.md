...
ssh appbuild 'cd .../desktop && ./build/syncv2test'   # 99 unit-тестов модели+синка
...
- `guitest` — 60/60, `syncv2test` — 91/99 (8 красных — те же, что и до перевода, см. ниже).
...
### Что сейчас красное
- **`dev_no == -1` отсеивает ВСЁ на инкрементной синхронизации без блока `device`.**
  `dnMap` заполняется только в ветке `cmd == "device"` приёма, а в файл индекса строка
  `["device-map",...]` после первого сопряжения не пишется (`aRecvAllWhenEmpty`/
  `aSendAllToEmptyPeer` создают `SyncIndex` с пустым `dnMap`). Поэтому на следующей
  синхронизации, если списки устройств не менялись, карта пуста, все `dev_no` дают -1 и
  события с удалениями молча выбрасываются. Проверено: с возвратом прежнего поведения
  (нет в карте — оставить как есть) большинство этих падений становится зелёным.
  Лечится записью карты (хотя бы тождественной) в индекс при первом сопряжении.
  Сюда же: «B принял ровно 1 новое событие (принято 2)» и «удаление дошло до C».
- Дедупликация одинаковых событий выполняется только на стороне сервера: у клиента дубликат
  остаётся и в памяти, и в файлах (5 красных в п.4 теста). Заготовка — закомментированный
  блок `TODO +++` в `Session::riEvent`.
- Трёхзвенная цепочка (A↔B, B↔C, удаление на A, снова A↔B) раньше роняла процесс в
  `aSendAllIncrement` (SEGV в `~MonthSyncData()` на выходе из корутины). Корутин больше нет,
  отдельного теста на цепочку в `syncv2test` тоже нет — при случае проверить заново.
